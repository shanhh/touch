var Util = {
	...
	numberFormat : function (number, decimals, dec_point, thousands_sep, dec_method) {
			...
			var n = !isFinite(+number) ? 0 : +number,
				...
				toFixedFix = function (n, prec) {
					...
					return '' + Util._Math.div(Math[dec_method](Util._Math.mul(n, k)), k);
				};
	...
	post: function (obj) {
		...
		var sendRquest = function () {
			...
			var params = (ajaxData.data && !self.isEmptyObject(ajaxData.data)) ? self.encodeURIJson(ajaxData.data) : {};
			request.send(params);
		};
		sendRquest();
	},
	...
	getiOSAPPVersion: function () {
		var match = (navigator.appVersion).match(/ltz-app-iphone_Apple_iPhone_(\d+)\.(\d+)\.?(\d+)?/),
				version;
		if (match !== undefined && match !== null) {
				version = [
						parseInt(match[1], 10),
